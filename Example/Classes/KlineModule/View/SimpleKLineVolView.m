...
- (void)p_drawTimeWithContent:(CGContextRef)ctx {
    ...
    for (NSInteger a = 0; a < self.timePointArray.count ; a ++) {
        ...
        NSString *timeString = [NSString gl_convertTimeStamp:([tempModel.id integerValue]) toFormatter:@"HH:mm"];
...
- (void)klineView:(KLineView *)view didMoveToPoint:(CGPoint)point selectedItemIndex:(NSInteger)index {
    ...
    NSString *dateString = [NSString gl_convertTimeStamp:([tempModel.id integerValue]) toFormatter:@"yy-MM-dd HH:mm"];
...
- (void)p_showDetailViewWithKLineModel:(KLineModel *)model isLeft:(BOOL)isLeft {
    ...
    // 时间
    DetailDataModel *timeModel = [[DetailDataModel alloc] initWithName:KLocalizedString(@"trade_commission_date", @"时间") desc:[NSString gl_convertTimeStamp:([model.id integerValue]) toFormatter:@"yy-MM-dd hh:mm"]];
    // 开
    DetailDataModel *openModel = [[DetailDataModel alloc] initWithName:KLocalizedString(@"trade_open", @"开") desc:[@([model.open doubleValue]) gl_numberToStringWithDecimalsLimit:self.kLineMainView.dataCenter.decimalsLimit]];
    // 高
    DetailDataModel *highModel = [[DetailDataModel alloc] initWithName:KLocalizedString(@"trade_high", @"高") desc:[@([model.high doubleValue]) gl_numberToStringWithDecimalsLimit:self.kLineMainView.dataCenter.decimalsLimit]];
    
    // 低
    DetailDataModel *lowModel = [[DetailDataModel alloc] initWithName:KLocalizedString(@"trade_low", @"低") desc:[@([model.low doubleValue]) gl_numberToStringWithDecimalsLimit:self.kLineMainView.dataCenter.decimalsLimit]];
    
    // 收
    DetailDataModel *closeModel = [[DetailDataModel alloc] initWithName:KLocalizedString(@"trade_closed", @"收") desc:[@([model.close doubleValue]) gl_numberToStringWithDecimalsLimit:self.kLineMainView.dataCenter.decimalsLimit]];
    
    // 量
    DetailDataModel *volModel = [[DetailDataModel alloc] initWithName:KLocalizedString(@"trade_amount_short", @"量") desc:[@([model.amount doubleValue]) stringValue]];
