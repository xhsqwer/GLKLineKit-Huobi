...
+ (NSMutableArray * _Nullable)convertToKlineModelArrayWithJsonData:(NSData * _Nonnull)jsonData {
    ...
    // 解析
    NSError *error = nil;
    NSArray *jsonArray =[(NSDictionary *)jsonData objectForKey:@"data"];
...
//            KLineModel *tempModel = [KLineModel objectFromDictionary:(NSDictionary *)obj];
//            if (tempModel) {
//
...
//        }];
        ...
        [jsonArray enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            KLineModel *tempModel = [KLineModel objectFromDictionary:(NSDictionary *)obj];
            if (tempModel) {
