...
- (void)p_getDataToKline {
    ...
    [SYYHuobiNetHandler requestHistoryKlineWithTag:self
                                           ...
                                           succeed:^(id respondObject) {
                                               ...
                                               [self.fullKLineView.dataCenter addMoreDataWithArray:dataArray isMergeModel:^BOOL(KLineModel * _Nonnull firstModel, KLineModel * _Nonnull secondModel) {
...
//                                               [self sendMessage];
                                               [self.fullKLineView gl_stopAnimating];
                                               // 更新上方当前信息
...
- (void)p_updateCurrentDetailData {
    ...
    if (tempModel) {
        ...
        self.currentPriceLabel.text = [@([tempModel.close doubleValue]) stringValue];
