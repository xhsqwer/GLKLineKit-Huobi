...
+ (double)p_averagePriceWithCount:(NSInteger)count endIndex:(NSInteger)index {
    ...
    for (NSInteger a = index; a > (index - count) ; a --) {
        KLineModel *tempModel = [DataCenter shareCenter].klineModelArray[a];
        sum += [tempModel.close doubleValue];
    }
...
+ (double)p_averageVolumeWithCount:(NSInteger)count endIndex:(NSInteger)index {
    ...
    for (NSInteger a = index; a > (index - count) ; a --) {
        KLineModel *tempModel = [DataCenter shareCenter].klineModelArray[a];
        sum += [tempModel.amount doubleValue];
    }
...
+ (double)p_std20WithCount:(NSInteger)count endIndex:(NSInteger)index {
    ...
    for (NSInteger a = index; a > (index - count) ; a --) {
        ...
        sum += pow(([tempModel.close doubleValue] - ma20),2);
    }
    // 开平方
...
+ (void)p_expmaWithIndex:(NSInteger)index {
    ...
    if (index == 0) {
        ...
        // 第一日的ema12为收盘价
        tempModel.ema12 = [tempModel.close doubleValue];
        tempModel.ema26 = [tempModel.close doubleValue];
        ...
        if (lastModel.ema12 && lastModel.ema26) {
            tempModel.ema12 = (2.0/13.0) * ([tempModel.close doubleValue]- lastModel.ema12) + lastModel.ema12;
            tempModel.ema26 = (2.0/27.0) * ([tempModel.close doubleValue]- lastModel.ema26) + lastModel.ema26;
...
+ (double)p_rsv9WithEndIndex:(NSInteger)endindex {
    ...
    for (NSInteger a = endindex; a >= startIndex; a --) {
        ...
        if ([tempModel.low doubleValue]< low9) {
            low9 = [tempModel.low doubleValue];
        }
        
        if ([tempModel.high doubleValue]> high9) {
            high9 = [tempModel.high doubleValue];
        }
        
        if (a == endindex) {
            close = [tempModel.close doubleValue];
        }
    }
...
+ (void)p_rsAtIndex:(NSInteger)atIndex {
    ...
    double diff = [tempModel.close doubleValue]- [lastModel.close doubleValue];
