...
//        NSAssert(diffValue > 0.0, @"最大值和最小值差值不能为0");
    ...
    for (NSInteger a = beginItemIndex; a <= endItemIndex; a ++) {
        ...
        // 设置涨跌颜色
        CGColorRef color = ([tempModel.open doubleValue]- [tempModel.close doubleValue]) >= 0 ? [self.config fallingColor].CGColor : [self.config risingColor].CGColor;
        // 设置画笔颜色
        ...
        // 实体线
        CGFloat openY = rect.size.height * (1.0f - ([tempModel.open doubleValue]- self.extremeValue.minValue) / diffValue) + rect.origin.y ;
        CGFloat closeY = rect.size.height * (1.0f - ([tempModel.close doubleValue]- self.extremeValue.minValue) / diffValue) + rect.origin.y;
        if (ABS(closeY - openY) <= 1.0f) {
            closeY = openY + 1.0f;
        }
        
        // 影线
        CGFloat highY = rect.size.height * (1.0f - ([tempModel.high doubleValue]- self.extremeValue.minValue) / diffValue) + rect.origin.y;
        CGFloat lowY = rect.size.height * (1.0f - ([tempModel.low doubleValue]- self.extremeValue.minValue) / diffValue) + rect.origin.y;
...
- (void)p_updateMinAndMaxValueWithBeginIndex:(NSInteger)beginIndex endIndex:(NSInteger)endIndex arguments:(NSDictionary *)arguments {
    ...
    for (NSInteger a = beginIndex; a <= endIndex; a ++) {
        ...
        KLineModel *tempModel = [DataCenter shareCenter].klineModelArray[a];
        if (tempModel) {
            if ([tempModel.high doubleValue]> maxValue) {
                maxValue = [tempModel.high doubleValue];
            }
            
            if ([tempModel.low doubleValue]< minValue) {
                minValue = [tempModel.low doubleValue];
            }
        }
    }
